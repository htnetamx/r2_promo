...
class PromoService {
    constructor() {
        this.repos = {
            mongoDB: new mongoDB_1.PromoRepositoryMongoDB(),
            mySQL: new mySQL_1.PromoRepositoryMySQL(),
        };
    }
    ...
    getAllPromo(numPerPage, page) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                var promises = [];
                const entries = Object.entries(this.repos);
                entries.forEach((entry) => promises.push(entry[1].getAllPromo(numPerPage, page)));
                let result_promises = yield Promise.all(promises);
                if (result_promises.length > 0) {
                    result_promises = result_promises.filter((i) => i !== null);
                    var succeses = [];
