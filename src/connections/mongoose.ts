...
export class MongooseConnection extends Connection {
  ...
  constructor(
    input: MongooseConnectionInputParams,
  ...
  private generateMongooseURI(input: MongooseConnectionInputParams): string {
    ...
    input.username
      ...
      : (auth = "");
    return (
      "mongodb://" + auth + input.host + ":" + input.port + "/" + input.database
    );
  }
    ...
    try {
      this.pool = (
        await mongoose.connect(
          this.uri,
          <MongooseConnectionOptions>this.options
        )
      ).connection;
      this.uri = this.uri;
